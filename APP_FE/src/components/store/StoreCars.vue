...
export default {
  ...
  setup() {
    ...
    const storeName = ref(decodeURIComponent(route.params.nome));
    const cars = ref([]);
    ...
    const fetchVehiclesByBrand = async () => {
      try {
        errorMessage.value = '';
        const response = await StoreService.getStoreWithVehicles(storeName.value);
        cars.value = response.data.vehicles
          .filter(vehicle => vehicle.brand === brandName.value)
          .map(vehicle => ({
            licensePlate: vehicle.licensePlate,
            brand: vehicle.brand,
            model: vehicle.model,
            store: response.data.store.nome,
          }));
      } catch (error) {
    ...
    const goBack = () => {
      router.push(`/store/page/${encodeURIComponent(storeName.value)}`);
    };
